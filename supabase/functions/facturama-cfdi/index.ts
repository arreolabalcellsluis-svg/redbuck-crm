...
function demoUuid(): string {
  ...
  return `DEMO-${hex()}-${hex()}-${hex()}-${hex()}${hex()}${hex()}`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Configurable: set FACTURAMA_ENV=demo for mock responses, =production for live API
    const FACTURAMA_ENV = Deno.env.get("FACTURAMA_ENV") || "sandbox";
    const isDemo = FACTURAMA_ENV === "demo";
    ...
    if (isDemo) {
      ...
      if (action === "test-connection") {
        return new Response(JSON.stringify({
          success: true,
          demo: true,
          profile: { Rfc: "DEMO000101AAA", TaxName: "MODO DEMO - Sin conexión real a Facturama", Status: "active" },
      ...
      if (action === "stamp") {
        ...
        const demoResponse = {
          ...
          Status: "active",
          Complement: { TaxStamp: { Uuid: uuid, Date: now, RfcProvCertif: "DEMO000101AAA", SatSeal: "DEMO_SEAL" } },
          _demo: true,
        };

        // Generate a simple demo XML
        const demoXml = `<?xml version="1.0" encoding="utf-8"?>
<cfdi:Comprobante xmlns:cfdi="http://www.sat.gob.mx/cfd/4" Version="4.0" Serie="${invoice.series}" Folio="${invoice.folio}" Fecha="${now}" Total="${invoice.total}" SubTotal="${invoice.subtotal}" Moneda="${invoice.currency || 'MXN'}" TipoDeComprobante="${invoice.invoice_type || 'I'}">
  <cfdi:Complemento>
    <tfd:TimbreFiscalDigital xmlns:tfd="http://www.sat.gob.mx/TimbreFiscalDigital" UUID="${uuid}" FechaTimbrado="${now}" RfcProvCertif="DEMO000101AAA" />
  </cfdi:Complemento>
  <!-- DOCUMENTO DEMO - NO VÁLIDO FISCALMENTE -->
</cfdi:Comprobante>`;

        // Upload demo XML
        const xmlPath = `xml/${invoice_id}.xml`;
        await supabase.storage.from("invoicing").upload(xmlPath, new Blob([demoXml], { type: "text/xml" }), { upsert: true });

        // Update invoice
        await supabase.from("invoices").update({
          uuid,
          status: "timbrada",
          pac_response: demoResponse,
          xml_path: xmlPath,
          pdf_path: "",
          issued_at: now,
          updated_at: now,
        ...
        return new Response(JSON.stringify({ success: true, uuid, stampData: demoResponse, demo: true }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (action === "cancel") {
        ...
        await supabase.from("invoice_cancellations").insert({
          ...
          canceled_by: body.canceled_by || "sistema (demo)",
          cancellation_ack_path: "",
        });

        return new Response(JSON.stringify({ success: true, cancelData: "Cancelación demo exitosa", demo: true }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (action === "generate-complement") {
        ...
        return new Response(JSON.stringify({ success: true, uuid: compUuid, demo: true }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }

      if (action === "download" || action === "download-complement") {
        return new Response(JSON.stringify({
          success: false,
          error: "En modo demo no se generan archivos PDF reales. El XML demo está disponible.",
          demo: true,
    ...
    if (!FACTURAMA_USERNAME || !FACTURAMA_PASSWORD) {
      throw new Error("Credenciales de Facturama no configuradas. Configura FACTURAMA_ENV=demo para modo pruebas.");
    }
    ...
    if (action === "test-connection") {
      const res = await facturama("/api/Cfdi/ProfileStatus");
    ...
    if (action === "stamp") {
      ...
      const { data: fiscal } = await supabase
        .from("fiscal_settings").select("*").limit(1).single();
      ...
      if (invoice.customer_id) {
        const { data: custFiscal } = await supabase
          .from("customer_fiscal_data").select("*").eq("customer_id", invoice.customer_id).single();
        if (custFiscal) {
          receiver = {
            Rfc: custFiscal.rfc,
    ...
    if (action === "generate-complement") {
      ...
      let compXmlPath = "";
      try {
        const xmlRes = await facturama(`/api/Cfdi/xml/issued/${compData.Id || compUuid}`, "GET");
        if (xmlRes.ok) {
          const xmlContent = await xmlRes.text();
      ...
      let compPdfPath = "";
      try {
        const pdfRes = await facturama(`/api/Cfdi/pdf/issued/${compData.Id || compUuid}`, "GET");
        if (pdfRes.ok) {
          const pdfBlob = await pdfRes.blob();
    ...
    if (action === "download") {
      ...
      const { data: invoice } = await supabase
        .from("invoices").select("xml_path, pdf_path, uuid").eq("id", invoice_id).single();
      if (!invoice) throw new Error("Factura no encontrada");
    ...
    if (action === "download-complement") {
      ...
      const { data: payment } = await supabase
        .from("payments").select("complement_xml_path, complement_pdf_path, complement_uuid").eq("id", payment_id).single();
      if (!payment) throw new Error("Pago no encontrado");
