 ...
 *
 * REUSES:
 * - demoProducts, demoAccountsReceivable, demoOrders (demo-data)
 * - DBAccountPayable (useAccountsPayable)
 * - IncomeStatement from cfoDashboardEngine
 */

import { demoProducts, demoAccountsReceivable, demoOrders } from '@/data/demo-data';
...
export function detectSlowInventory(thresholdDays = 60): SlowInventoryItem[] {
  // Determine last sale date per product from orders
  const lastSaleMap = new Map<string, string>();
  for (const order of demoOrders) {
    for (const item of order.items) {
      // Match by product name
      const product = demoProducts.find(p => item.productName.includes(p.name.split(' ').slice(0, 2).join(' ')));
      if (product) {
  ...
  for (const p of demoProducts.filter(pr => pr.active)) {
    ...
    const diasSinVenta = lastSale
      ...
      : 180; // If no sale found, assume 180 days
...
export function detectLowMarginProducts(thresholdPct = 25): LowMarginProduct[] {
  const results: LowMarginProduct[] = [];

  for (const p of demoProducts.filter(pr => pr.active)) {
    const margenLista = ((p.listPrice - p.cost) / p.listPrice) * 100;
    const margenMinimo = ((p.minPrice - p.cost) / p.minPrice) * 100;
    const stock = totalStock(p.stock);
...
export function detectCapitalConsumingClients(thresholdDays = 30): CapitalConsumingClient[] {
  // Group AR by customer
  const clientMap = new Map<string, { name: string; balance: number; total: number; count: number; maxDays: number }>();

  for (const ar of demoAccountsReceivable) {
  ...
  for (const [id, data] of clientMap) {
    ...
    // Estimate days: if overdue use that, otherwise estimate from balance/total ratio
    ...
    if (diasPromedio >= thresholdDays || data.balance > 30000) {
      results.push({
        ...
        saldoPorCobrar: data.balance,
        totalFacturado: data.total,
        diasPromedioCobro: diasPromedio,
        facturasPendientes: data.count,
...
export function detectExcessInventory(): ExcessInventoryItem[] {
  // Estimate monthly sales per product from orders (simple heuristic)
  ...
  for (const p of demoProducts.filter(pr => pr.active)) {
    const stock = totalStock(p.stock);
    const monthlySales = (salesMap.get(p.id) ?? 0.5); // assume 0.5/month if no data
    // Recommended: 2 months of sales + lead time coverage
...
// ─── 6. Summary & Alerts ────────────────────────────────────────
export function calcLeakSummary(payables: DBAccountPayable[]): LeakSummary {
  const slow = detectSlowInventory();
  const lowMargin = detectLowMarginProducts();
  const clients = detectCapitalConsumingClients();
  const excess = detectExcessInventory();
  ...
  return {
    ...
    capitalBajoMargen,
    capitalClientesLentos,
    capitalExcesoInventario,
    presionPagos30d,
