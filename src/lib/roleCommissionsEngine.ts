 ...
 * Commission is generated by ORDERS and paid only when the order is fully paid (balance = 0).
 */

import {
  type VendorKPI, type CommissionConfig, type ScoreWeights, type ScoreLevel,
  type TeamMember, type ARRecord,
...
export interface GerenteConfig {
  enabled: boolean;
  teamGoalBonuses: { min_pct: number; bonus: number }[];
...
export interface CobranzaConfig {
  enabled: boolean;
  collectedBonusRate: number;
  overdueRecoveryBonusRate: number;
  goalBonuses: { min_pct: number; bonus: number }[];
}

export interface AdminConfig {
  enabled: boolean;
  orderProcessedBonus: number;
...
export function calcGerenteCommission(
  ...
  const gerente = teamMembers.find(u => u.role === 'gerencia_comercial' && u.active);

  const teamPaidSales = vendorKPIs.reduce((s, k) => s + k.paidSales, 0);
  const teamGoal = vendorKPIs.reduce((s, k) => s + k.goalSales, 0);
  const teamPct = teamGoal > 0 ? Math.round((teamPaidSales / teamGoal) * 100) : 0;
  const teamOrders = vendorKPIs.reduce((s, k) => s + k.paidOrders, 0);
  ...
  const collectionPct = teamPaidSales > 0 ? (teamCollections / teamPaidSales) * 100 : 0;
  ...
  if (teamMargin >= config.marginThreshold) {
    const amount = teamPaidSales * (config.marginBonusPct / 100);
    bonuses.push({
      label: 'Bono margen equipo',
      amount,
      detail: `Margen ${teamMargin.toFixed(1)}% ≥ ${config.marginThreshold}% (${config.marginBonusPct}% de ventas pagadas)`,
    });
  }
  ...
  return {
    ...
    kpis: {
      'Ventas pagadas equipo': teamPaidSales,
...
export function calcAdminCommission(
  ...
  const admin = teamMembers.find(u => u.role === 'administracion' && u.active);
  const teamPaidSales = vendorKPIs.reduce((s, k) => s + k.paidSales, 0);
  const teamGoal = vendorKPIs.reduce((s, k) => s + k.goalSales, 0);
  const teamPct = teamGoal > 0 ? Math.round((teamPaidSales / teamGoal) * 100) : 0;
  ...
  if (errorRate < 10 && processedOrders > 0) {
    const effBonus = teamPaidSales * (config.efficiencyBonusPct / 100);
    bonuses.push({
      label: 'Bono eficiencia',
      amount: effBonus,
      detail: `Tasa error ${errorRate.toFixed(1)}% < 10% (${config.efficiencyBonusPct}% de ventas pagadas)`,
    });
  }
...
export function applyVendorPenalties(
  ...
  // Uncollected penalty based on paid vs total sales
  if (kpi.sales > 0) {
    const uncollectedPct = ((kpi.sales - kpi.paidSales) / kpi.sales) * 100;
    if (uncollectedPct > config.uncollectedThreshold) {
      const amount = commission * (config.uncollectedPenaltyPct / 100);
      penalties.push({
        label: 'Castigo pedidos sin cobrar',
        amount,
...
export function buildVendorResults(
  ...
  return vendorKPIs.map(kpi => {
    ...
    const bonuses: RoleCommissionResult['bonuses'] = [
      { label: 'Comisión base', amount: c.baseAmount, detail: `${c.baseRate}% de ${fmt(c.baseSales)} (pedidos pagados)` },
    ...
    return {
      ...
      kpis: {
        'Ventas totales': kpi.sales,
        'Ventas pagadas': kpi.paidSales,
        'Meta': kpi.goalSales,
        'Avance': `${kpi.progressSales}%`,
        'Pedidos': kpi.orders,
        'Pedidos pagados': kpi.paidOrders,
...
export function calcExecutiveSummary(
  ...
  const totalPaidSales = vendorKPIs.reduce((s, k) => s + k.paidSales, 0);
  const teamMargin = vendorKPIs.length > 0
    ? vendorKPIs.reduce((s, k) => s + k.marginAvg, 0) / vendorKPIs.length : 0;
  const totalGrossProfit = totalPaidSales * (teamMargin / 100);
  ...
  return {
    ...
    totalPaidSales,
    totalGrossProfit,
    commissionToSalesRatio: totalPaidSales > 0 ? (grandTotal / totalPaidSales) * 100 : 0,
