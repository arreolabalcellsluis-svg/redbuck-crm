...
export default function QuotationsPage() {
  const { currentRole, quotations, addQuotation, updateQuotationStatus, getNextFolio, consumeFolio, vendorSeries, orders, setOrders, setReceivables, registerPayment } = useAppContext();
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="mb-4">
            ...
            <select onChange={e => { if (e.target.value) { addItem(e.target.value); e.target.value = ''; } }} className="w-full px-3 py-2 rounded-lg border bg-card text-sm">
              <option value="">Seleccionar producto...</option>
              {demoProducts.filter(p => p.active).map(p => <option key={p.id} value={p.id}>{p.sku} — {p.name} — {fmt(p.listPrice)}</option>)}
            </select>
          </div>
