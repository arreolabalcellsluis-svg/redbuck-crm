...
function mapRow(row: any): DBOrder {
  return {
    ...
    shipping_images: Array.isArray(row.shipping_images) ? row.shipping_images : [],
    invoice_number_manual: row.invoice_number_manual || '',
    invoice_date_manual: row.invoice_date_manual,
    invoice_pdf_url: row.invoice_pdf_url || '',
    edit_history: Array.isArray(row.edit_history) ? row.edit_history : [],
...
export function useAddOrder() {
  ...
  return useMutation({
    mutationFn: async (order: Omit<DBOrder, 'id' | 'created_at' | 'updated_at'>) => {
      ...
      if (error) throw error;
      return mapRow({ ...payload, created_at: timestamp, updated_at: timestamp });
    },
    onSuccess: () => {
